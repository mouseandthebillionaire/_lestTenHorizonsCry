...
public class explorationSynthHandle {
    ...
    [DllImport("explorationSynth", CallingConvention = CallingConvention.StdCall)]
    ...
    internal explorationSynthHandle(int key) {
        PluginKey = key;
    }
    
    ~explorationSynthHandle() {
    ...
    private static explorationSynthHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (explorationSynthHandle)gch.Target;
    }
...
public class explorationSynthHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static explorationSynthHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("explorationSynthHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                explorationSynthHelper helper = go.AddComponent<explorationSynthHelper>();
                helper.Plugin = new explorationSynthHandle(key);
                return helper;
            }
            return go.GetComponent<explorationSynthHelper>();
        }
    }

    public explorationSynthHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            explorationSynthHandle.RegisterGlobalTransport(null);
        }
    }
}
