...
public class SampleSynthHandle {
    ...
    [DllImport("SampleSynth", CallingConvention = CallingConvention.StdCall)]
    ...
    internal SampleSynthHandle(int key) {
        PluginKey = key;
    }
    ...
    private static SampleSynthHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (SampleSynthHandle)gch.Target;
    }
...
public class SampleSynthHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static SampleSynthHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("SampleSynthHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                SampleSynthHelper helper = go.AddComponent<SampleSynthHelper>();
                helper.Plugin = new SampleSynthHandle(key);
                return helper;
            }
            return go.GetComponent<SampleSynthHelper>();
        }
    }

    public SampleSynthHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            SampleSynthHandle.RegisterGlobalTransport(null);
        }
    }
}
