...
public class instEffectChainHandle {
    ...
    [DllImport("instEffectChain", CallingConvention = CallingConvention.StdCall)]
    ...
    internal instEffectChainHandle(int key) {
        PluginKey = key;
    }
    
    ~instEffectChainHandle() {
    ...
    private static instEffectChainHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (instEffectChainHandle)gch.Target;
    }
...
public class instEffectChainHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static instEffectChainHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("instEffectChainHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                instEffectChainHelper helper = go.AddComponent<instEffectChainHelper>();
                helper.Plugin = new instEffectChainHandle(key);
                return helper;
            }
            return go.GetComponent<instEffectChainHelper>();
        }
    }

    public instEffectChainHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            instEffectChainHandle.RegisterGlobalTransport(null);
        }
    }
}
