...
public class LTHCsynthHandle {
    ...
    [DllImport("LTHCsynth", CallingConvention = CallingConvention.StdCall)]
    ...
    internal LTHCsynthHandle(int key) {
        PluginKey = key;
    }
    ...
    private static LTHCsynthHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (LTHCsynthHandle)gch.Target;
    }
...
public class LTHCsynthHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static LTHCsynthHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("LTHCsynthHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                LTHCsynthHelper helper = go.AddComponent<LTHCsynthHelper>();
                helper.Plugin = new LTHCsynthHandle(key);
                return helper;
            }
            return go.GetComponent<LTHCsynthHelper>();
        }
    }

    public LTHCsynthHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            LTHCsynthHandle.RegisterGlobalTransport(null);
        }
    }
}
